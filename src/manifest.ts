        ...
        Type.Object(
          {
            ...
            id: Type.String({
              description: "Stable id — must match the entry key.",
              title: "Id",
            }),
            label: Type.Optional(Type.String({ title: "Display name" })),
  ...
  tools: {
    check_residency: tool.runtime({
      ...
      handler: ({ classification, region, tenant }, runtime) => {
        const violation = createResidencyGuard(runtime.policy).inspect({
          classification,
          region,
          tenant,
        });
    ...
    erasure_dry_run: tool.runtime({
      ...
      handler: async ({ subjectId, tenant }, runtime) => {
        ...
        return JSON.stringify(
          Object.fromEntries(
            Object.entries(report.byEraser).map(([name, entry]) => [
              name,
              {
      ...
      handler: async ({ subjectId, tenant }, runtime) => {
        ...
        return JSON.stringify({
          byCollector: Object.fromEntries(
            Object.entries(bundle.byCollector).map(([name, entry]) => [
              name,
              {
                classification: entry.classification,
          ...
          errors: bundle.errors.map(
            ({ collector, error }) => `${collector}: ${error.message}`,
          ),
        });
      },
      input: Type.Object({
        ...
        imports: [
          {
            ...
            names: ["createCompliancePolicy", "createResidencyGuard"],
          },
          {
            from: "@absolutejs/compliance",
