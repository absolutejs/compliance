...
describe("messaging consent", () => {
  ...
  test("supplies a dispatch policy that enforces the exact scope", async () => {
    ...
    const context = {
      ...
      message: {
        ...
        tenant: scope.tenant,
        to: { address: scope.recipient, transport: "sms" as const },
      },
    };
    ...
    expect(
      await policy.evaluate({
        ...
        message: {
          ...context.message,
          to: { address: scope.recipient, transport: "rcs" as const },
        },
      }),
  ...
  test("requires consent for every possible fallback transport", async () => {
    ...
    const message = {
      ...
      fallbacks: [{ transport: "sms" as const }],
      tenant: scope.tenant,
      to: { address: scope.recipient, transport: "rcs" as const },
    };
    expect(
  ...
  test("exports recipient evidence and purges only historical records by default", async () => {
    ...
    await ledger.revoke(scope, { at: 200, source: "twilio-stop" });
    expect(
      await store.exportRecipient({ recipient: scope.recipient }),
    ).toHaveLength(2);
...
describe("createResidencyGuard", () => {
  ...
  test("throws ResidencyViolation on region mismatch", () => {
    ...
    expect(() => guard.check({ classification: "pii", region: "eu" })).toThrow(
      ResidencyViolation,
    );
  });
